...
export class UsageService extends Context.Service<
  UsageService,
  {
    ...
    /**
     * Claude's subscription quota, which is a different question from
...
export const make = Effect.gen(function* () {
  ...
  const claudeUsageLimits = yield* makeClaudeUsageLimitsReader;
      ...
      Effect.catchCause(
        () =>
          new ClaudeUsageError({
            ...
            detail: "Server settings could not be read, so the Claude home is unknown.",
          }),
      ),
    );
    // Same as the transcript scan: `Path` is provided from the instance we
    // already hold so the service stays context-free.
