...
/**
 * Usage of a finished assistant message. Pi reports input, output, and cache
 * tokens per message; their sum is the context size after that message.
 */
function readPiUsage(
  message: unknown,
): { input: number; output: number; cacheRead: number; cacheWrite: number } | undefined {
  if (!message || typeof message !== "object") return undefined;
  const record = message as { readonly role?: unknown; readonly usage?: unknown };
  ...
  return result.input + result.output + result.cacheRead + result.cacheWrite > 0
    ? result
    : undefined;
}
...
export const makePiAgentAdapter = Effect.fn("makePiAgentAdapter")(function* (
  ...
  // Extension notifications (for example the /fast toggle) as a short assistant note.
  const emitNote = (context: SessionContext, text: string) =>
    Effect.gen(function* () {
      const itemId = RuntimeItemId.make(`pi-note-${yield* randomId}`);
      const base = {
        provider: PROVIDER,
        providerInstanceId: instanceId,
        threadId: context.threadId,
  ...
  const handleDialog = (context: SessionContext, event: PiRpcEnvelope) =>
    Effect.gen(function* () {
      ...
      if (event.method === "notify") {
        const text = (event as { readonly message?: unknown }).message;
        if (typeof text === "string" && text.trim()) yield* emitNote(context, text.trim());
        return;
      }
      if (
  ...
  const handleEvent = (context: SessionContext, event: PiRpcEnvelope) =>
    Effect.gen(function* () {
      ...
      if (event.type === "message_end") {
        const usage = readPiUsage((event as { readonly message?: unknown }).message);
        if (usage) {
          ...
          yield* emit({
            ...
            payload: {
              usage: {
                usedTokens,
                ...(context.contextWindow ? { maxTokens: context.contextWindow } : {}),
      ...
      if (isPiRpcToolExecutionStartEvent(event)) {
        ...
        // Surface what the tool is doing: the shell command for bash, the file
        // for edits/writes, and the path in the title for reads and the rest.
        const args =
        ...
        const title =
          filePath && itemType !== "file_change" ? `${event.toolName} ${filePath}` : event.toolName;
        const tool: ToolState = {
          itemId: RuntimeItemId.make(event.toolCallId),
          itemType,
          title,
          ...(command ? { command } : {}),
          ...(filePath && itemType === "file_change" ? { filePath } : {}),
          output: undefined,
        };
      ...
      if (isPiRpcToolExecutionEndEvent(event)) {
        ...
        yield* emit({
          ...
          payload: {
            ...
            title: tool.title ?? event.toolName,
            // The completion replaces the start payload in the timeline, so the
            // command and file travel again, and the output goes where the web
            // client reads it from.
            data: {
              result: event.result.details,
              ...(tool.command ? { command: tool.command } : {}),
              ...(tool.filePath ? { filePath: tool.filePath } : {}),
              ...(tool.output ? { rawOutput: tool.output } : {}),
            },
          },
        });
      ...
      Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          ...
          context.contextWindow =
            typeof state.model?.contextWindow === "number" && state.model.contextWindow > 0
              ? Math.round(state.model.contextWindow)
              : undefined;
    ...
    Effect.gen(function* () {
      ...
      return yield* context.commandLock.withPermit(
        Effect.gen(function* () {
          ...
          if (!steering && promptMessage.trim() === "/compact") {
            // Pi's manual compaction is an RPC command, not a prompt. Run it as
            // its own turn so the compaction items and the meter update.
            yield* context.client.request({ type: "compact" }).pipe(
              Effect.mapError((cause) => mapClientError(input.threadId, "compact", cause)),
              Effect.tapError((error) =>
                finishTurn(context, { state: "failed", errorMessage: error.message }),
              ),
            );
            yield* finishTurn(context, { state: "completed" });
            return {
              threadId: input.threadId,
              turnId,
          ...
          if (!steering && promptMessage.trimStart().startsWith("/")) {
            // Extension commands run without an agent turn. Settle the turn
            // when no agent_start follows, so the thread does not wait forever.
            yield* Effect.sleep("3 seconds").pipe(
              Effect.andThen(
                Effect.suspend(() =>
                  context.activeTurnId === turnId && !context.agentStarted
                    ? finishTurn(context, { state: "completed" })
                    : Effect.void,
                ),
              ),
              Effect.forkIn(ownerScope),
    ...
    Effect.gen(function* () {
      ...
      yield* context.commandLock.withPermit(
        context.client.request({ type: "clear_queue" }).pipe(
          // Older Pi builds (0.84) have no clear_queue; abort alone is enough there.
          Effect.ignore,
