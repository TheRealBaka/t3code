...
const PI_BINARY = "pi";
const PI_RPC_TIMEOUT = "45 seconds";
const PI_VERSION_TIMEOUT = "30 seconds";
...
/**
 * Pi's `thinkingLevelMap` only lists the levels a model remaps (for example
 * `minimal` -> `low`); unlisted levels pass through unchanged. The supported
 * set is therefore the global level list run through that map.
 */
function modelThinkingLevels(
  model: UnknownRecord,
  globalLevels: ReadonlyArray<string>,
): ReadonlyArray<string> {
  if (!isRecord(model.thinkingLevelMap)) return [];
  const map = model.thinkingLevelMap;
  // A listed key maps to its value, or is unsupported when the value is null.
  const fromGlobal = globalLevels.flatMap((level) => {
    if (!Object.prototype.hasOwnProperty.call(map, level)) return [level];
    const mapped = stringValue(map[level]);
    return mapped ? [mapped] : [];
  });
  const fromMap = Object.values(map).flatMap((value) => {
    const level = stringValue(value);
    return level ? [level] : [];
  });
  return uniqueStrings([...fromGlobal, ...fromMap]);
}
...
export function buildPiAgentModels(input: {
  ...
  for (const entry of models) {
    ...
    seen.add(slug);
    const advertisedThinkingLevels = modelThinkingLevels(entry, input.thinkingLevels ?? []);
